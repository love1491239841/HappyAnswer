package com.example.administrator.happyanswer.utils;

public class Constants {
    public final static String URL = "http://192.168.43.144:8000/";
    public final static String LOGIN_URL = URL+"login";
    public final static String RGGISTER_URL = URL+"register";
    public final static String NEWS_URL = URL+"news";
    public final static String Answer_URL = URL+"title";
    public final static String SETAnswer_URL = URL+"setRecord";
    public final static String Record_URL = URL+"getRecord";
    public final static String NEWSIMG_URL = "http://192.168.43.144:8000/static/images/";

}
